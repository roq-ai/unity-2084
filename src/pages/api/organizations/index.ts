...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.time_off_request?.length > 0) {
      const create_time_off_request = body.time_off_request;
      body.time_off_request = {
        create: create_time_off_request,
      };
    } else {
      delete body.time_off_request;
    }
